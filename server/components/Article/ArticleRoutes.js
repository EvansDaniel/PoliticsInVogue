...
const ArticleRoutes = function (ArticleDataService) {
    ...
    const articlesCallBack = function (err, articles) {
        if(err) {
            return routeUtils.handle500Error(err, next);
        }
        if(!articles) {
            return routeUtils.handle404Error(next);
        }
    }
    ...
    const getArticleHandle = function (req, res, next) {
        routeUtils.debuggingHelper(req, res, next, function (req, res, next) {
            const reqObj = {
                normalizedTitle: req.query.normalizedTitle,
                createdAt: req.query.createdAt
            };
            ArticleDataService.getArticleByTitleAndDate(reqObj, function (err, article) {
                if(err) {
                    return routeUtils.handle500Error(err, next);
                }
                if(!article) {
                ...
                articleObj.year = createdAt.getFullYear();
                articleObj.month = createdAt.getMonth() + 1;
                articleObj.day = createdAt.getDate();
            ...
            return res.json({
                ...
                timeToReadInMin: 12//ArticleUtils.timeToReadInMin(body)
            });*/
        });
    };

    const getPlacementArticleHandle = function (req, res, next) {
        ArticleDataService.getPlacedArticles(req.param.placement, );
    };

    const getCreateArticleHandle = function (req, res, next) {
        routeUtils.debuggingHelper(req, res, next, function (req, res, next) {
            ArticleDataService.getArticle()
        });
    };
